...
export function OnlineBoard({
  ...
  useEffect(() => {
    if (playerId) {
      const channel = pusherClient.subscribe(`game-${playerId}`);
      
      return () => {
        pusherClient.unsubscribe(`game-${playerId}`);
      };
    }
  }, [playerId]);
  ...
  useEffect(() => {
    const fetchGameState = async () => {
      try {
        const { gameState: gameStateData, players } = await getGameState(roomId);
        const playerColour = await getPlayerColor(playerId);
        if (playerColour) setPlayerColor(playerColour);

        const parsedGameState = gameStateData as unknown as {
          board: string;
          status: string;
          ...
          canPromotePawn: string;
          whiteTime: number;
          blackTime: number;
        };

        const gameState: GameState = {
          board: parsedGameState.board ? JSON.parse(parsedGameState.board) : [],
          status: parsedGameState.status as "in-progress" | "checkmate" | "draw" | "resigned" | "promote",
          currentPlayer: parsedGameState.currentPlayer as "white" | "black",
          lastMove: parsedGameState.lastMove ? JSON.parse(parsedGameState.lastMove) : null,
          movingPiece: parsedGameState.movingPiece ? JSON.parse(parsedGameState.movingPiece) : null,
          kingCheckOrMoved: parsedGameState.kingCheckOrMoved ? JSON.parse(parsedGameState.kingCheckOrMoved) : undefined,
          rookMoved: parsedGameState.rookMoved ? JSON.parse(parsedGameState.rookMoved) : undefined,
          eliminatedPieces: parsedGameState.eliminatedPieces ? JSON.parse(parsedGameState.eliminatedPieces) : undefined,
          isKingInCheck: parsedGameState.isKingInCheck || "noCheck",
          canPromotePawn: parsedGameState.canPromotePawn ? JSON.parse(parsedGameState.canPromotePawn) : null,
          whiteTime: parsedGameState.whiteTime || 600,
          blackTime: parsedGameState.blackTime || 600
        };
        updateGameState(gameState);

        const recievedPlayers: Player[] = players.map((player: any) => ({
          id: player.id,
          color: player.color,
  ...
  if (!gameState) return <LoadingBoard />;
        ...
        {gameState.board.map((row: any[], rowIndex: number) =>
          row.map((cell: PieceType, colIndex: number) => (
            <div
              ...
              ${
                String(gameState.board[rowIndex][colIndex]) === gameState.isKingInCheck
