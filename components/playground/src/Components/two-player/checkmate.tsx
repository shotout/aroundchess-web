...
import type { typeCheckMate } from "../../types/chess";
import JSConfetti from "js-confetti";
import { useEffect, useState } from "react";

export const CheckMate = () => {
  const { isCheckMate, currentPlayer, isKingInCheck } = useChessStore((state) => state);
  const [isClient, setIsClient] = useState(false);

  useEffect(() => {
    const isGameOver = 
      (typeof window !== "undefined") && 
      (isCheckMate !== "noCheckMate" || isKingInCheck !== "noCheck");
  ...
  if (!isClient || (isCheckMate === "noCheckMate" && isKingInCheck === "noCheck")) {
    return null;
  }
