...
import { EndgameTopic, Resource } from "@/components/analysis/training-plan/training-topics/types"
...
interface EndgameTopicPageProps {
  topic: EndgameTopic
  topicId: string
}
...
export default function EndgameTopicPage({ topic, topicId }: EndgameTopicPageProps) {
  const initialPosition = endgamePositions[topicId]
          ...
          <Card className="p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-muted-foreground">Key strategic and tactical ideas in this position</p>
              </div>

              <div className="grid grid-cols-2 gap-6">
                <div>
                  <h3 className="font-medium mb-2">Strategic Ideas</h3>
                  <ul className="space-y-2">
                    {topic.theoreticalKnowledge?.slice(0, 2).map((idea, index) => (
                ...
                <div>
                  <h3 className="font-medium mb-2">Tactical Ideas</h3>
                  <ul className="space-y-2">
                    {topic.practicalTips?.slice(0, 2).map((idea, index) => (
                      <li key={index} className="text-muted-foreground flex items-start gap-2">
                        <span className="select-none">•</span>
                        <span>{idea}</span>
                      </li>
              ...
              <Tabs defaultValue="overview" className="w-full mt-4">
                ...
                <TabsContent value="overview" className="mt-6">
                  <div className="space-y-6">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold mb-3">Theoretical Knowledge</h3>
                      <ul className="space-y-2">
                        {topic.theoreticalKnowledge?.map((concept, index) => (
                          <li key={index} className="text-muted-foreground flex items-start gap-2">
                ...
                <TabsContent value="patterns" className="mt-6">
                  <div className="space-y-6">
                    <div>
                      ...
                      <ul className="space-y-2">
                        {topic.fundamentalPositions?.map((position, index) => (
                          <li key={index} className="text-muted-foreground flex items-start gap-2">
                            <Target className="h-4 w-4 mt-1" />
                            <span>{position}</span>
                          </li>
                    ...
                    <div>
                      ...
                      <ul className="space-y-2">
                        {topic.winningTechniques?.map((technique, index) => (
                          <li key={index} className="text-muted-foreground flex items-start gap-2">
                            <Target className="h-4 w-4 mt-1" />
                            <span>{technique}</span>
                          </li>
