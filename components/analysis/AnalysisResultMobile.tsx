...
const AnalysisResultMobile = ({handleHeightChange}: {handleHeightChange: (height: number) => void}) => {
  const {
    pgn: storePgn,
    dataAnalysis,
    hideDiv,
        ...
        <div className="flex items-center gap-[8px]">
          <button type="button">
            <svg width="28" height="24" viewBox="0 0 28 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M24.0391 12H4M4 12L11 19M4 12L11 5" stroke="#1E1E1E" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </button>
          <div className="flex flex-col">
            <h3 className="font-semibold">Analysis Result</h3>
            <span className="text-[12px]">{moment(gameInfo?.date).format("DD/MM/YYYY")}</span>
          </div>
        </div>
  ...
  useEffect(() => {
    if (refWrap.current) {
      handleHeightChange(refWrap.current.clientHeight - 32);
    }
...
export default AnalysisResultMobile;
