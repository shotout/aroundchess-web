...
import { SkillIconType } from "./util/PerformanceHelper";
...
type StrengthsWeaknessesSectionProps = {
  strengthsData: SkillStrengthItem[];
  weaknessesData: SkillWeaknessItem[];
};

const getIconForSkillType = (iconType: SkillIconType) => {
  switch (iconType) {
    case "Calculation":
    case "Tactical":
...
type StrengthCardProps = {
  item: SkillStrengthItem;
};

const StrengthCard: React.FC<StrengthCardProps> = ({ item }) => {
...
type WeaknessCardProps = {
  item: SkillWeaknessItem;
};

const WeaknessCard: React.FC<WeaknessCardProps> = ({ item }) => {
  return (
    <Card className="bg-white rounded-lg p-3 flex items-center">
      <div className="mr-3 bg-gray-100 rounded-full h-10 w-10 min-w-10 flex justify-center items-center">
