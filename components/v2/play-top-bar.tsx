  ...
  return (
    <div className="flex items-center gap-[4px] min-[1600px]:gap-[6px]">
      <Image
        ...
        width={22}
        height={22}
        className={`w-[20px] h-[20px] min-[1600px]:w-[22px] min-[1600px]:h-[22px] object-contain shrink-0 ${muted ? "grayscale opacity-70" : ""}`}
      />
    ...
    <div className="flex flex-col gap-[4px]">
      <div className="flex items-center gap-[4px]">
        ...
        {infoText ? (
          ...
          <Image src="/images/v2/play/information.png" alt="info" width={13} height={13} className="w-[13px] h-[13px] object-contain shrink-0" />
        )}
      </div>
...
export function EloScoreLink({ className = "" }: { className?: string }) {
  ...
  return (
    <>
      ...
      <button
        ...
        className={`flex items-center gap-[6px] text-[#221AE9] font-medium text-[14px] hover:underline ${className}`}
      >
        <Image
...
export function LeaderboardStatsCardMobile({
  elo,
  rank,
  movedUp,
  canJoin,
  ...
  const [showEloModal, setShowEloModal] = useState(false);
  const handleLeaderboardClick = useLeaderboardNav();
      ...
      <div className="sm:hidden bg-[linear-gradient(to_bottom,#EFFAFF,#C9E9FC)] rounded-xl border border-[#E5E7EB] px-[16px] py-[14px] shadow-xl">
        <div className="flex items-center justify-between gap-[8px] mb-[12px]">
          ...
          >
            ...
            />
            <span className="font-bold text-[20px] text-[#111827] truncate">
              Leaderboard
            </span>
        ...
        >
          ...
          <div className="flex items-start justify-between">
            <MobileStatItem label="Your ELO" infoText={ELO_INFO} infoAlign="left">
              {showJoinCover ? (
            ...
            <MobileStatItem label="Your Rank" infoText={RANK_INFO} infoAlign="center">
...
export function PlayTopBar({ streak, elo, rank, movedUp, canJoin, gamesRemaining, isInactive }: PlayTopBarProps) {
  ...
  return (
    <div data-tour-anchor="play-top-bar" className="flex flex-col">
      ...
      <div className="hidden sm:block bg-[linear-gradient(to_bottom,white,#E6F7FE)] rounded-xl border px-[12px] min-[1600px]:px-[24px] py-[14px] shadow-xl pb-5">
        <div className="flex flex-wrap items-center gap-x-[10px] min-[1600px]:gap-x-[14px] gap-y-[12px]">
          <div className="flex items-center gap-[8px] min-[1600px]:gap-[20px] shrink-0 pt-2">
            ...
            <button
              ...
              className="flex items-center gap-[5px] pl-1 cursor-pointer"
            ...
            >
              <Image
                ...
                width={32}
                height={38}
                className="w-[28px] h-[34px] min-[1600px]:w-[32px] min-[1600px]:h-[38px] object-contain"
              />
          ...
          <div
            className={`relative flex-1 min-w-[300px] flex flex-col items-center gap-[15px] rounded-md px-[6px] py-2 ${
              showJoinCover ? "bg-[#E5E7EB]" : "bg-white"
            } ${showFreezeCover ? "pb-[24px]" : ""}`}
          >
            {showFreezeCover && <StatsCover gamesRemaining={gamesRemaining} />}
            <div className="flex flex-wrap items-center justify-center gap-x-[8px] min-[1600px]:gap-x-[34px] gap-y-[6px]">
          ...
          <Link
            href="/leaderboard"
            onClick={handleLeaderboardClick}
            className="flex items-center gap-[8px] font-bold text-[18px] min-[1600px]:text-xl text-[#111827] hover:text-[#221AE9] transition-colors shrink-0"
          >
            <Image
              src="/images/v2/play/leaderboard.png"
              alt="Leaderboard"
              width={48}
              height={44}
              className="h-[38px] min-[1600px]:h-[44px] w-auto object-contain"
            />
            Leaderboard
