...
const Break = () => <br className="hidden sm:inline" />;

const CONTENT: Record<
  GameLeaveGuardType,
  { title: ReactNode; body: ReactNode; confirmLabel: string }
> = {
  resign: {
  ...
  leave: {
    title: (
      <>
        ...
        <Break /> leave this Game?
      </>
    ),
    body: (
      <>
        Leaving this game will lead you to the previous page and end
        <Break /> your current game. Your progress will not be saved.
      </>
    ),
  ...
  return (
    <div className="fixed inset-0 z-[500] flex items-center justify-center bg-black/50 p-4">
      <div className="relative w-full max-w-[545px] bg-white rounded-2xl shadow-2xl px-[24px] py-[28px]">
        ...
        <p className="text-center text-[15px] leading-[150%] text-[#111827] mb-[20px]">
          {content.body}
        </p>

        <div className="flex flex-col-reverse sm:flex-row gap-[12px] sm:gap-[16px]">
          <button
            onClick={onCancel}
