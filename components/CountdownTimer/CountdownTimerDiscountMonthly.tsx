...
const getTimeRemaining = (endTime: number) => {
  ...
  const days = Math.floor((total / (1000 * 60 * 60)) % 7);
...
export default function CountdownTimerDiscountMonthly() {
  const { profile } = useProfileStore();
  const [nextDiscountTimeMonthly, setNextDiscountTime] = useState<number>(() => {
    const saved = localStorage.getItem("nextDiscountTimeMonthly");
    const deadline =
    ...
    if (saved && parseInt(saved, 10) > deadline) {
      ...
      const next = deadline;
      localStorage.setItem("nextDiscountTimeMonthly", next.toString());
      return next;
    }
  });

  const [timeLeft, setTimeLeft] = useState(getTimeRemaining(nextDiscountTimeMonthly));
