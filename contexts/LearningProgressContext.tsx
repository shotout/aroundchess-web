...
export function LearningProgressProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const savedProgress = localStorage.getItem('chessFundamentalsOverallProgress')
  ...
  useEffect(() => {
    ...
    localStorage.setItem('chessFundamentalsOverallProgress', newProgress.toString())
  ...
  return (
    <LearningProgressContext.Provider value={{ sections, progress, toggleSectionCompletion, isCompleted, completeLesson }}>
      {children}
