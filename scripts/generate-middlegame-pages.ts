...
function generatePageContent(topic: string) {
  const componentName = topic
    .split('-')
    .map(word => word.charAt(0).toUpperCase() + word.slice(1))
    .join('')

  const importName = topic.replace(/-./g, x => x[1].toUpperCase())
...
import { ${importName} } from "@/components/analysis/training-plan/training-topics/middlegame/${topic}"
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">{${importName}.title}</h1>
          <p className="text-muted-foreground">{${importName}.description}</p>
        </div>
      </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <Badge variant="secondary">{${importName}.difficulty}</Badge>
                  <div className="flex items-center gap-1 text-muted-foreground">
                    <Clock className="h-4 w-4" />
                    <span>{${importName}.estimatedTime}</span>
                  </div>
                </div>
                <TooltipProvider>
                  <Tooltip>
              ...
              <Tabs defaultValue="overview" className="w-full">
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <ul className="space-y-2">
                        {${importName}.objectives?.map((objective, index) => (
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        {${importName}.prerequisites?.map((prereq, index) => (
                    ...
                    <div>
                      ...
                      <ul className="space-y-2">
                        {${importName}.strategicConcepts?.map((concept, index) => (
                          <li key={index} className="text-muted-foreground flex items-start gap-2">
                ...
                <TabsContent value="patterns" className="space-y-4">
                  <div className="space-y-6">
                    <div>
                      ...
                      <ul className="space-y-2">
                        {${importName}.patterns?.map((pattern, index) => (
                          <li key={index} className="text-muted-foreground flex items-start gap-2">
                            <Target className="h-4 w-4 mt-1" />
                            <span>{pattern}</span>
                          </li>
                    ...
                    <div>
                      ...
                      <ul className="space-y-2">
                        {${importName}.tacticalMotifs?.map((motif, index) => (
                          <li key={index} className="text-muted-foreground flex items-start gap-2">
                            <Target className="h-4 w-4 mt-1" />
                            <span>{motif}</span>
                          </li>
                ...
                <TabsContent value="resources" className="space-y-4">
                  {${importName}.resources?.map((resource, index) => (
        ...
        <div className="space-y-6">
          ...
          <Card className="p-6">
            ...
            <div className="space-y-2">
              {${importName}.relatedTopics?.map((topic, index) => (
...
console.log('Generated page.tsx files for all middlegame topics') 