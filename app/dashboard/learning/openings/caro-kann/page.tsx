...
import { caroKann } from "@/components/analysis/training-plan/training-topics/openings/caro-kann"
...
export default function CaroKannPage() {
  const [currentVariation, setCurrentVariation] = useState(caroKann.variations[0])
  const [game, setGame] = useState<Chess>(new Chess())
  const [position, setPosition] = useState("rnbqkbnr/pp1ppppp/2p5/8/4P3/8/PPPP1PPP/RNBQKBNR w KQkq - 0 2")
  ...
  function resetPosition() {
    const newGame = new Chess("rnbqkbnr/pp1ppppp/2p5/8/4P3/8/PPPP1PPP/RNBQKBNR w KQkq - 0 2")
    setGame(newGame)
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">{caroKann.title}</h1>
          <p className="text-muted-foreground">{caroKann.description}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <Card className="p-6">
            ...
            <OpeningIdeas ideas={openingIdeas['caro-kann']} />
            
            <div className="space-y-4">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <Badge variant="secondary">{caroKann.difficulty}</Badge>
                  <div className="flex items-center gap-1 text-muted-foreground">
                    <Clock className="h-4 w-4" />
                    <span>{caroKann.estimatedTime}</span>
                  </div>
                </div>
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent>
                      <p>Reset to initial position (after 1.e4 c6)</p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              </div>
              ...
              <Tabs defaultValue="overview" className="w-full">
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <ul className="space-y-2">
                        {caroKann.objectives.map((objective, index) => (
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        {caroKann.prerequisites?.map((prereq, index) => (
                ...
                <TabsContent value="variations" className="space-y-4">
                  {caroKann.variations.map((variation, index) => (
                    <Card
                      key={index}
                ...
                <TabsContent value="resources" className="space-y-4">
                  {caroKann.resources.map((resource, index) => (
        ...
        <div className="space-y-6">
          ...
          <Card className="p-6">
            ...
            <div className="space-y-2">
              {caroKann.relatedTopics?.map((topic, index) => (
