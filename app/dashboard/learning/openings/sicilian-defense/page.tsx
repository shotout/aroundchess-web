...
import { sicilianDefense } from "@/components/analysis/training-plan/training-topics/openings/sicilian-defense"
...
export default function SicilianDefensePage() {
  const [currentVariation, setCurrentVariation] = useState(sicilianDefense.variations[0])
  const [game, setGame] = useState<Chess>(new Chess())
  const [position, setPosition] = useState("rnbqkbnr/pp1ppppp/8/2p5/4P3/8/PPPP1PPP/RNBQKBNR w KQkq c6 0 2")
  ...
  function resetPosition() {
    const newGame = new Chess("rnbqkbnr/pp1ppppp/8/2p5/4P3/8/PPPP1PPP/RNBQKBNR w KQkq c6 0 2")
    setGame(newGame)
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">{sicilianDefense.title}</h1>
          <p className="text-muted-foreground">{sicilianDefense.description}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <Card className="p-6">
            ...
            <OpeningIdeas ideas={openingIdeas['sicilian-defense']} />
            
            <div className="space-y-4">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <Badge variant="secondary">{sicilianDefense.difficulty}</Badge>
                  <div className="flex items-center gap-1 text-muted-foreground">
                    <Clock className="h-4 w-4" />
                    <span>{sicilianDefense.estimatedTime}</span>
                  </div>
                </div>
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent>
                      <p>Reset to initial position (after 1.e4 c5)</p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              </div>
              ...
              <Tabs defaultValue="overview" className="w-full">
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <ul className="space-y-2">
                        {sicilianDefense.objectives.map((objective, index) => (
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        {sicilianDefense.prerequisites?.map((prereq, index) => (
                ...
                <TabsContent value="variations" className="space-y-4">
                  {sicilianDefense.variations.map((variation, index) => (
                    <Card
                      key={index}
                ...
                <TabsContent value="resources" className="space-y-4">
                  {sicilianDefense.resources.map((resource, index) => (
        ...
        <div className="space-y-6">
          ...
          <Card className="p-6">
            ...
            <div className="space-y-2">
              {sicilianDefense.relatedTopics?.map((topic, index) => (
