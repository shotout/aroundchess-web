...
import { scotchGame } from "@/components/analysis/training-plan/training-topics/openings/scotch-game"
...
export default function ScotchGamePage() {
  const [currentVariation, setCurrentVariation] = useState<OpeningTopic['variations'][0]>(scotchGame.variations[0])
  const [game, setGame] = useState<Chess>(new Chess())
  const [position, setPosition] = useState("r1bqkbnr/pppp1ppp/2n5/4p3/3PP3/5N2/PPP2PPP/RNBQKB1R b KQkq d3 0 3")
  ...
  function resetPosition() {
    const newGame = new Chess("r1bqkbnr/pppp1ppp/2n5/4p3/3PP3/5N2/PPP2PPP/RNBQKB1R b KQkq d3 0 3")
    setGame(newGame)
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">{scotchGame.title}</h1>
          <p className="text-muted-foreground">{scotchGame.description}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <Card className="p-6">
            ...
            <OpeningIdeas ideas={openingIdeas['scotch-game']} />
            
            <div className="space-y-4">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <Badge variant="secondary">{scotchGame.difficulty}</Badge>
                  <div className="flex items-center gap-1 text-muted-foreground">
                    <Clock className="h-4 w-4" />
                    <span>{scotchGame.estimatedTime}</span>
                  </div>
                </div>
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent>
                      <p>Reset to initial position (after 1.e4 e5 2.Nf3 Nc6 3.d4)</p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              </div>
              ...
              <Tabs defaultValue="overview" className="w-full">
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <ul className="space-y-2">
                        {scotchGame.objectives.map((objective, index) => (
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        {scotchGame.prerequisites?.map((prereq, index) => (
                ...
                <TabsContent value="variations" className="space-y-4">
                  {scotchGame.variations.map((variation, index) => (
                    <Card
                      key={index}
                ...
                <TabsContent value="resources" className="space-y-4">
                  {scotchGame.resources.map((resource, index) => (
        ...
        <div className="space-y-6">
          ...
          <Card className="p-6">
            ...
            <div className="space-y-2">
              {scotchGame.relatedTopics?.map((topic, index) => (
