...
import { viennaGame } from "@/components/analysis/training-plan/training-topics/openings/vienna-game"
...
export default function ViennaGamePage() {
  const [currentVariation, setCurrentVariation] = useState<OpeningTopic['variations'][0]>(viennaGame.variations[0])
  const [game, setGame] = useState<Chess>(new Chess())
  const [position, setPosition] = useState("rnbqkbnr/pppp1ppp/8/4p3/4P3/2N5/PPPP1PPP/R1BQKBNR b KQkq - 1 2")
  ...
  function resetPosition() {
    const newGame = new Chess("rnbqkbnr/pppp1ppp/8/4p3/4P3/2N5/PPPP1PPP/R1BQKBNR b KQkq - 1 2")
    setGame(newGame)
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">{viennaGame.title}</h1>
          <p className="text-muted-foreground">{viennaGame.description}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <Card className="p-6">
            ...
            <OpeningIdeas ideas={openingIdeas['vienna-game']} />
            
            <div className="space-y-4">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <Badge variant="secondary">{viennaGame.difficulty}</Badge>
                  <div className="flex items-center gap-1 text-muted-foreground">
                    <Clock className="h-4 w-4" />
                    <span>{viennaGame.estimatedTime}</span>
                  </div>
                </div>
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent>
                      <p>Reset to initial position (after 1.e4 e5 2.Nc3)</p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              </div>
              ...
              <Tabs defaultValue="overview" className="w-full">
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <ul className="space-y-2">
                        {viennaGame.objectives.map((objective, index) => (
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        {viennaGame.prerequisites?.map((prereq, index) => (
                ...
                <TabsContent value="variations" className="space-y-4">
                  {viennaGame.variations.map((variation, index) => (
                    <Card
                      key={index}
                ...
                <TabsContent value="resources" className="space-y-4">
                  {viennaGame.resources.map((resource, index) => (
        ...
        <div className="space-y-6">
          ...
          <Card className="p-6">
            ...
            <div className="space-y-2">
              {viennaGame.relatedTopics?.map((topic, index) => (
