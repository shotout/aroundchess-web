...
import { nimzoIndian } from "@/components/analysis/training-plan/training-topics/openings/nimzo-indian"
...
export default function NimzoIndianPage() {
  const [currentVariation, setCurrentVariation] = useState(nimzoIndian.variations[0])
  const [game, setGame] = useState<Chess>(new Chess())
  const [position, setPosition] = useState("rnbqk2r/pppp1ppp/4pn2/8/1bPP4/2N5/PP2PPPP/R1BQKBNR w KQkq - 2 4")
  ...
  function resetPosition() {
    const newGame = new Chess("rnbqk2r/pppp1ppp/4pn2/8/1bPP4/2N5/PP2PPPP/R1BQKBNR w KQkq - 2 4")
    setGame(newGame)
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">{nimzoIndian.title}</h1>
          <p className="text-muted-foreground">{nimzoIndian.description}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <Card className="p-6">
            ...
            <OpeningIdeas ideas={openingIdeas['nimzo-indian']} />
            
            <div className="space-y-4">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <Badge variant="secondary">{nimzoIndian.difficulty}</Badge>
                  <div className="flex items-center gap-1 text-muted-foreground">
                    <Clock className="h-4 w-4" />
                    <span>{nimzoIndian.estimatedTime}</span>
                  </div>
                </div>
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent>
                      <p>Reset to initial position (after 1.d4 Nf6 2.c4 e6 3.Nc3 Bb4)</p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              </div>
              ...
              <Tabs defaultValue="overview" className="w-full">
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <ul className="space-y-2">
                        {nimzoIndian.objectives.map((objective, index) => (
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        {nimzoIndian.prerequisites?.map((prereq, index) => (
                ...
                <TabsContent value="variations" className="space-y-4">
                  {nimzoIndian.variations.map((variation, index) => (
                    <Card
                      key={index}
                ...
                <TabsContent value="resources" className="space-y-4">
                  {nimzoIndian.resources.map((resource, index) => (
        ...
        <div className="space-y-6">
          ...
          <Card className="p-6">
            ...
            <div className="space-y-2">
              {nimzoIndian.relatedTopics?.map((topic, index) => (
