...
import { initiative } from "@/components/analysis/training-plan/training-topics/middlegame/initiative"
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <Badge variant="secondary">{initiative.difficulty}</Badge>
                  <div className="flex items-center gap-1 text-muted-foreground">
                    <Clock className="h-4 w-4" />
                    <span>{initiative.estimatedTime}</span>
                  </div>
                </div>
                <TooltipProvider>
                  <Tooltip>
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        {initiative.prerequisites?.map((prereq, index) => (
                    ...
                    <div>
                      ...
                      <ul className="space-y-2">
                        {initiative.strategicConcepts?.map((concept, index) => (
                          <li key={index} className="text-muted-foreground flex items-start gap-2">
                ...
                <TabsContent value="patterns" className="space-y-4">
                  <div className="space-y-6">
                    <div>
                      ...
                      <ul className="space-y-2">
                        {initiative.patterns?.map((pattern, index) => (
                          <li key={index} className="text-muted-foreground flex items-start gap-2">
                            <Target className="h-4 w-4 mt-1" />
                            <span>{pattern}</span>
                          </li>
                    ...
                    <div>
                      ...
                      <ul className="space-y-2">
                        {initiative.tacticalMotifs?.map((motif, index) => (
                          <li key={index} className="text-muted-foreground flex items-start gap-2">
                            <Target className="h-4 w-4 mt-1" />
                            <span>{motif}</span>
                          </li>
        ...
        <div className="space-y-6">
          ...
          <Card className="p-6">
            ...
            <div className="space-y-2">
              {initiative.relatedTopics?.map((topic, index) => (
