...
import { breakthroughSacrifices } from "@/components/analysis/training-plan/training-topics/middlegame/breakthrough-sacrifices"
import { middlegamePositions } from "@/components/analysis/training-plan/training-topics/middlegame/positions"
import { Chess, Square } from "chess.js"
import { PositionIdeas, positionIdeas } from "@/components/learn/middlegame-strategy/PositionIdeas"
...
export default function BreakthroughSacrificesPage() {
  const initialPosition = middlegamePositions['breakthrough-sacrifices']
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">{breakthroughSacrifices.title}</h1>
          <p className="text-muted-foreground">{breakthroughSacrifices.description}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <Card className="p-6">
            ...
            <PositionIdeas ideas={positionIdeas['breakthrough-sacrifices']} />
            
            <div className="space-y-4">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <Badge variant="secondary">{breakthroughSacrifices.difficulty}</Badge>
                  <div className="flex items-center gap-1 text-muted-foreground">
                    <Clock className="h-4 w-4" />
                    <span>{breakthroughSacrifices.estimatedTime}</span>
                  </div>
                </div>
                <TooltipProvider>
                  <Tooltip>
              ...
              <Tabs defaultValue="overview" className="w-full">
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <ul className="space-y-2">
                        {breakthroughSacrifices.objectives?.map((objective, index) => (
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        {breakthroughSacrifices.prerequisites?.map((prereq, index) => (
                    ...
                    <div>
                      ...
                      <ul className="space-y-2">
                        {breakthroughSacrifices.strategicConcepts?.map((concept, index) => (
                          <li key={index} className="text-muted-foreground flex items-start gap-2">
                ...
                <TabsContent value="patterns" className="space-y-4">
                  <div className="space-y-6">
                    <div>
                      ...
                      <ul className="space-y-2">
                        {breakthroughSacrifices.patterns?.map((pattern, index) => (
                          <li key={index} className="text-muted-foreground flex items-start gap-2">
                            <Target className="h-4 w-4 mt-1" />
                            <span>{pattern}</span>
                          </li>
                    ...
                    <div>
                      ...
                      <ul className="space-y-2">
                        {breakthroughSacrifices.tacticalMotifs?.map((motif, index) => (
                          <li key={index} className="text-muted-foreground flex items-start gap-2">
                            <Target className="h-4 w-4 mt-1" />
                            <span>{motif}</span>
                          </li>
                ...
                <TabsContent value="resources" className="space-y-4">
                  {breakthroughSacrifices.resources?.map((resource, index) => (
        ...
        <div className="space-y-6">
          ...
          <Card className="p-6">
            ...
            <div className="space-y-2">
              {breakthroughSacrifices.relatedTopics?.map((topic, index) => (
