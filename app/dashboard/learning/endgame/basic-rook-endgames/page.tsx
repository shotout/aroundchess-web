...
import { basicrookendgames } from "@/components/analysis/training-plan/training-topics/endgame/basic-rook-endgames"
...
export default function BasicRookEndgamesPage() {
  const initialPosition = endgamePositions['basic-rook-endgames']
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">{basicrookendgames.title}</h1>
          <p className="text-muted-foreground">{basicrookendgames.description}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <Badge variant="secondary">{basicrookendgames.difficulty}</Badge>
                  <div className="flex items-center gap-1 text-muted-foreground">
                    <Clock className="h-4 w-4" />
                    <span>{basicrookendgames.estimatedTime}</span>
                  </div>
                </div>
                <TooltipProvider>
                  <Tooltip>
              ...
              <Tabs defaultValue="overview" className="w-full">
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <ul className="space-y-2">
                        {basicrookendgames.objectives?.map((objective, index) => (
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        {basicrookendgames.prerequisites?.map((prereq, index) => (
                    ...
                    <div>
                      ...
                      <ul className="space-y-2">
                        {basicrookendgames.fundamentalPositions?.map((position, index) => (
                          <li key={index} className="text-muted-foreground flex items-start gap-2">
                ...
                <TabsContent value="patterns" className="space-y-4">
                  <div className="space-y-6">
                    <div>
                      ...
                      <ul className="space-y-2">
                        {basicrookendgames.winningTechniques?.map((technique, index) => (
                          <li key={index} className="text-muted-foreground flex items-start gap-2">
                            <Target className="h-4 w-4 mt-1" />
                            <span>{technique}</span>
                          </li>
                    ...
                    <div>
                      ...
                      <ul className="space-y-2">
                        {basicrookendgames.commonMistakes?.map((mistake, index) => (
                          <li key={index} className="text-muted-foreground flex items-start gap-2">
                            <Target className="h-4 w-4 mt-1" />
                            <span>{mistake}</span>
                          </li>
                ...
                <TabsContent value="resources" className="space-y-4">
                  {basicrookendgames.resources?.map((resource, index) => (
        ...
        <div className="space-y-6">
          <Card className="p-6">
            ...
            <Link href="/dashboard/learning/endgame/basic-rook-endgames/learn">
          ...
          <Card className="p-6">
            ...
            <div className="space-y-2">
              {basicrookendgames.relatedTopics?.map((topic, index) => (
