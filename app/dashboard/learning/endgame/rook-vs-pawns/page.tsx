...
import { rookvspawns } from "@/components/analysis/training-plan/training-topics/endgame/rook-vs-pawns"
...
export default function RookVsPawnsPage() {
  const initialPosition = endgamePositions['rook-vs-pawns']
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">{rookvspawns.title}</h1>
          <p className="text-muted-foreground">{rookvspawns.description}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <Badge variant="secondary">{rookvspawns.difficulty}</Badge>
                  <div className="flex items-center gap-1 text-muted-foreground">
                    <Clock className="h-4 w-4" />
                    <span>{rookvspawns.estimatedTime}</span>
                  </div>
                </div>
                <TooltipProvider>
                  <Tooltip>
              ...
              <Tabs defaultValue="overview" className="w-full">
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <ul className="space-y-2">
                        {rookvspawns.objectives?.map((objective, index) => (
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        {rookvspawns.prerequisites?.map((prereq, index) => (
                    ...
                    <div>
                      ...
                      <ul className="space-y-2">
                        {rookvspawns.fundamentalPositions?.map((position, index) => (
                          <li key={index} className="text-muted-foreground flex items-start gap-2">
                ...
                <TabsContent value="patterns" className="space-y-4">
                  <div className="space-y-6">
                    <div>
                      ...
                      <ul className="space-y-2">
                        {rookvspawns.winningTechniques?.map((technique, index) => (
                          <li key={index} className="text-muted-foreground flex items-start gap-2">
                            <Target className="h-4 w-4 mt-1" />
                            <span>{technique}</span>
                          </li>
                    ...
                    <div>
                      ...
                      <ul className="space-y-2">
                        {rookvspawns.commonMistakes?.map((mistake, index) => (
                          <li key={index} className="text-muted-foreground flex items-start gap-2">
                            <Target className="h-4 w-4 mt-1" />
                            <span>{mistake}</span>
                          </li>
                ...
                <TabsContent value="resources" className="space-y-4">
                  {rookvspawns.resources?.map((resource, index) => (
        ...
        <div className="space-y-6">
          <Card className="p-6">
            ...
            <Link href="/dashboard/learning/endgame/rook-vs-pawns/learn">
          ...
          <Card className="p-6">
            ...
            <div className="space-y-2">
              {rookvspawns.relatedTopics?.map((topic, index) => (
