...
export default function BasicCheckmatesPage() {
  const initialPosition = endgamePositions['basic-checkmates']
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">{basiccheckmates.title}</h1>
          <p className="text-muted-foreground">{basiccheckmates.description}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <Badge variant="secondary">{basiccheckmates.difficulty}</Badge>
                  <div className="flex items-center gap-1 text-muted-foreground">
                    <Clock className="h-4 w-4" />
                    <span>{basiccheckmates.estimatedTime}</span>
                  </div>
                </div>
                <TooltipProvider>
                  <Tooltip>
              ...
              <Tabs defaultValue="overview" className="w-full">
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <ul className="space-y-2">
                        {basiccheckmates.objectives?.map((objective: string, index: number) => (
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        {basiccheckmates.prerequisites?.map((prereq: string, index: number) => (
                    ...
                    <div>
                      ...
                      <ul className="space-y-2">
                        {basiccheckmates.fundamentalPositions?.map((position: string, index: number) => (
                          <li key={index} className="text-muted-foreground flex items-start gap-2">
                ...
                <TabsContent value="patterns" className="space-y-4">
                  <div className="space-y-6">
                    <div>
                      ...
                      <ul className="space-y-2">
                        {basiccheckmates.winningTechniques?.map((technique: string, index: number) => (
                          <li key={index} className="text-muted-foreground flex items-start gap-2">
                            <Target className="h-4 w-4 mt-1" />
                            <span>{technique}</span>
                          </li>
                    ...
                    <div>
                      ...
                      <ul className="space-y-2">
                        {basiccheckmates.commonMistakes?.map((mistake: string, index: number) => (
                          <li key={index} className="text-muted-foreground flex items-start gap-2">
                            <Target className="h-4 w-4 mt-1" />
                            <span>{mistake}</span>
                          </li>
                ...
                <TabsContent value="resources" className="space-y-4">
                  {basiccheckmates.resources?.map((resource: Resource, index: number) => (
        ...
        <div className="space-y-6">
          <Card className="p-6">
            ...
            <Link href="/dashboard/learning/endgame/basic-checkmates/learn">
          ...
          <Card className="p-6">
            ...
            <div className="space-y-2">
              {basiccheckmates.relatedTopics?.map((topic: string, index: number) => (
